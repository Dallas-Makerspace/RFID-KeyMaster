...
class PiGpioZeroInterface(Interface):

#
#  PiGpioZeroInterface.PinTranslate is used to convert .ini file IO assignments to
...
# seperate configuration arguments and strip whitespace.  
        ...
        iopin = PiGpioZeroInterface.PinTranslate.get(str.lower(position[0]))
        ...
        elif PiGpioZeroInterface.IOAssignment.get(iopin) == iou:
            
            pass
    #
	#  use gpiozero inputdeecice and output device classes to check and set pins.
	#	Set pullup/pulldown based on logic sense(?) or all pullup??
	#
	#	plan for transition to using library pin assingment setting.
	#
	
        elif PiGpioZeroInterface.IOAssignment.get(iopin) == "U":
    
            if iou == "I":
                gpio.setup(iopin, gpio.IN, gpio.PUD_UP)
                PiGpioZeroInterface.IOAssignment.update({iopin : "I"})
            
            elif iou == "O":
                gpio.setup(iopin, gpio.OUT)
                PiGpioZeroInterface.IOAssignment.update({iopin : "O"})
            ...
            logging.debug("ERROR: IO Assignment Conflict for",function,
                ...
                " and is already set to ",PiGpioZeroInterface.IOAssignment.get(iopin))
...
# can probably just reference gpio.cleanup directly
    #	modify to close all active pins - will this have consequences for running machines?
